$(function(){
    ...
    btnGetPackages.on('click',function(){
        ...
        frmScanRfid.ajaxSubmit({
            url:'http://localhost:8888/rms/ajax.php?action=get-packages',
            ...
            success:function(response){
                ...
                $.ajax({
                    url:'http://localhost:8888/rms/ajax.php?action=get-wrong-packages',
                    success:function(response){
                        response=jQuery.parseJSON(response);
                        for(i=0;i<response.length;i++){
                            var current_station="";
                            ...
                            $("#packages-list").append("<tr data-date='" + response[i].sent_date + "' class='danger' id='" + response[i].uid +  "'> <td>" + response[i].uid + "</td><td>" + response[i].source + "</td><td id='status'> Belongs to " + response[i].destination + " Wrongly Unloaded <a href='update_rfid.html'>Update Status</a> </td></tr>");
                        }
                    }
                });
            }
